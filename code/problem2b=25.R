...
while(m<1001)
{
  x<-X[m,1:30]
  y<-Y[m,1]
  g_old<-g_old+norm((y-x%*%beta),type="2")*norm((y-x%*%beta),type="2")/2
  m<-m+1
}
g_old<-g_old/n
...
b<-1
gradient<-matrix(0,30,1)
while(b<26)
{
  j=round(runif(1,1,1000))
  x=X[j,1:30]
  y=Y[j,1]
  gradient<-gradient+t(((x%*%beta-y)%*%x))
  b<-b+1
}
...
while(m<1001)
{
  x<-X[m,1:30]
  y<-Y[m,1]
  g<-g+norm((y-x%*%beta),type="2")*norm((y-x%*%beta),type="2")/2
  m<-m+1
}
g<-g/n
g_beta<-c(g_beta,g)
